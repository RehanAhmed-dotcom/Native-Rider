...
const TrackMap = ({route, navigation}) => {
    ...
    PostAPiwithToken({url: 'start-ride', Token: token}, formdata)
      .then(res => {
        ...
        if (res.status === 'success') {
          Toast.show({
            type: 'success',
            text1: 'Your ride started successfully!',
            text2: res.message,
  ...
  const CompleteRideApi = () => {
    ...
    PostAPiwithToken({url: 'complete-ride', Token: token}, formdata)
      .then(res => {
        setIsLoading(false);
        console.log('my complete ride response', res);
        if (res.status === 'success') {
          navigation.navigate('Drawer_R', {screen: 'Bookings_R'});
  ...
  const startLocationTracking = async () => {
    ...
    if (!hasPermission) {
      Toast.show({
        ...
        text2: 'Please enable location services to track your ride.',
        topOffset: Platform.OS === 'ios' ? 50 : 0,
      });
      return;
    }
    ...
    watchIdRef.current = Geolocation.watchPosition(
      position => {
        ...
        setPickupCords(smoothedLocation);
  ...
  }, [isRideStarted, item.status, pickupCords]);
    ...
    <View style={styles.container}>
      ...
      <RBSheet
        ...
        <TouchableOpacity
          ...
          <Text style={styles.payButtonText}>Complete Your Ride</Text>
        </TouchableOpacity>
      </RBSheet>
      <Modal
        transparent={true}
