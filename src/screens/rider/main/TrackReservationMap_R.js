...
const TrackReservationMap_R = ({navigation, route}) => {
  ...
  const StartReservationApi = () => {
    ...
    PostAPiwithToken({url: 'start-reservation', Token: token}, formdata)
      .then(res => {
        setIsLoading(false);
        console.log('res of start reserv fd ', JSON.stringify(res));
        if (res.status === 'success') {
          Toast.show({
            type: 'success',
            text1: 'Your reservation started successfully!',
            text2: res.message,
  ...
  //       formdata.append('reservation_id', item.id);
  //       setIsLoading(true);
  //       PostAPiwithToken({ url: 'payment-request-reservation', Token: token }, formdata)
  //           .then(res => {
  //               setIsLoading(false);
  //               console.log('res of request payment ', JSON.stringify(res));
  //               if (res.status === 'success') {
  //                   navigation.navigate('Drawer_R', { screen: 'Reservation_R' });
  //                   bottomSheetRef.current.close();
  //                   Toast.show({
  //                       type: 'success',
  //                       text1: 'Request for payment sent!',
  //                       text2: res.message,
  //                       topOffset: Platform.OS === 'ios' ? 50 : 0,
  //                       visibilityTime: 3000,
  //                       autoHide: true,
  ...
  const CompleteResApi = () => {
    ...
    PostAPiwithToken({url: 'compelete-reservation', Token: token}, formdata)
      .then(res => {
        setIsLoading(false);
        console.log('res of Complete res ', JSON.stringify(res));
        if (res.status === 'success') {
          navigation.navigate('Drawer_R', {screen: 'Reservation_R'});
          bottomSheetRef.current.close();
          Toast.show({
            type: 'success',
            text1: 'Complete reservation',
            text2: res.message,
            topOffset: Platform.OS === 'ios' ? 50 : 0,
            visibilityTime: 3000,
            autoHide: true,
  ...
  const startLocationTracking = async () => {
    ...
    watchIdRef.current = Geolocation.watchPosition(
      position => {
        ...
        if (calculateDistance(smoothedLocation, droplocationCords) < 0.1) {
    ...
    <View style={styles.container}>
            ...
            <View
              ...
              <View style={{marginLeft: wp(2)}}>
                <Text style={[styles.usernameText, {color: Colors.black}]}>
                  {item?.rider?.name || 'Unknown Driver'}
                </Text>
                <Text
                  style={{
                    fontSize: 13,
      ...
      {isRideStarted || item.status === 'started' ? (
        <TouchableOpacity
          ...
          <Text style={styles.buttonText}>End Your Reservation</Text>
        </TouchableOpacity>
      ) : (
        <TouchableOpacity
          style={styles.startButton}
          ...
          <Text style={styles.buttonText}>Start Your Reservation</Text>
        </TouchableOpacity>
      )}
      ...
      <RBSheet
        ...
        <TouchableOpacity
          ...
          <Text style={styles.payButtonText}>Complete Your Reservation</Text>
        </TouchableOpacity>
      </RBSheet>
      <Modal
        transparent={true}
...
export default TrackReservationMap_R;
