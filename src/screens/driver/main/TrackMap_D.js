...
const TrackMap_D = ({route, navigation}) => {
  ...
  const fetchPaymentRequest = () => {
    ...
    PostAPiwithToken(
      {url: 'check-ride-status', Token: user?.api_token},
      formdata,
    )
      .then(res => {
        console.log('response dataaaaaaaaa status', JSON.stringify(res));
        if (res.status === 'success' && !hasInitiatedPayment) {
          if (res.offer_status === 'accepted') {
      ...
      <RBSheet
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            navigation.navigate('BookingReachedMap', {
              item,
