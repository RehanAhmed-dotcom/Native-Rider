...
const ReachedMap = ({navigation, route}) => {
  const user = useSelector(state => state.user.user);
  const [isloading, setIsLoading] = useState(false);
  const [promoCode, setPromoCode] = useState('');
  const {item, distance} = route.params;
  console.log('my item data aaa', item);
  const {top, bottom} = useSafeAreaInsets();
  const refRBSheet = useRef();
  useEffect(() => {
    // Open the bottom sheet when the screen is loaded
  ...
  const _paymentIntentApi = () => {
    ...
    formdata.append(
      'amount',
      item.driver_offer ? item.driver_offer : item?.final_price,
    );
    ...
    PostAPiwithToken({url: 'payment-intent', Token: user?.api_token}, formdata)
      .then(res => {
        console.log('ressss of intent api', JSON.stringify(res));
  ...
  const _paymentConfirmApi = response => {
    ...
    formdata.append('reservation_id', response.reservation_id);
    ...
    PostAPiwithToken({url: 'confirm-payment', Token: user?.api_token}, formdata)
      .then(res => {
        ...
        if (res.status == 'success') {
          ...
          // navigation.navigate('ResFeedBack',{item})
        } else {
        ...
        <View style={{marginHorizontal: wp(6), marginTop: wp(3)}}>
          <View
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Image
                source={images.chatpic1}
                resizeMode="contain"
                style={styles.driverimg}
                borderRadius={wp(5)}
              />
              <View style={{marginLeft: wp(2)}}>
                <Text style={styles.usernameText}>{item?.driver?.name}</Text>
                <Text
                  style={{
                    fontSize: 13,
                    fontFamily: fonts.medium,
